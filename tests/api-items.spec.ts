...
test.describe('API - Items Endpoints', () => {
  ...
  // API-100: GET /api/items listado sin filtros
  ...
  // API-101: GET /api/items filtrado por texto (q)
  ...
  // API-102: GET /api/items con filtros combinados
  test('API-102: should filter items with multiple filters', async ({ request }) => {
    ...
    // Verificar que la API responde correctamente con múltiples filtros
    // (La lógica de filtrado depende de la implementación del backend)
  });
  ...
  test('should return empty array for non-matching filters', async ({ request }) => {
    const response = await request.get(`${baseURL}/api/items?q=xyznonexistent999`);
...
test.describe('API - Item Availability Endpoints', () => {
  ...
  // API-110: GET disponibilidad de un ítem existente
  test('API-110: should return availability for existing item', async ({ request }) => {
    ...
    // Verificar estructura de disponibilidad
    // (Depende de la implementación, puede ser un array de fechas, un objeto con disponibilidad, etc.)
  });
  ...
  // API-111: GET disponibilidad de ítem inexistente (404)
