...
test.describe('Security Tests', () => {
  ...
  // SEC-400: Mitigación de SQL injection en campos de búsqueda
  ...
  test('SEC-401: should mitigate XSS in rental form fields', async ({ page }) => {
    ...
    await page.locator('input[name="start"]').fill(startDateStr);
    await page.locator('input[name="end"]').fill(endDateStr);

    const reserveButton = page.getByRole('button', { name: /Reserve|Book/i });
    ...
    if (await nameInput.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const submitButton = page.getByRole('button', { name: /Submit|Confirm|Book|Complete/i });
  ...
  // SEC-402: Forzar HTTPS en panel de administración
  ...
  // SEC-403: Contraseñas no visibles en respuestas o logs
  ...
  // Tests adicionales de seguridad general
  test('should have security headers', async ({ page }) => {
    ...
    if (response) {
      ...
      // Verificar headers de seguridad (algunos pueden no estar en desarrollo)
      ...
      // Solo advertir si faltan, no fallar el test en localhost
  ...
  test('should prevent clickjacking with X-Frame-Options', async ({ page }) => {
    ...
    if (response && !baseURL.includes('localhost')) {
      const headers = response.headers();
